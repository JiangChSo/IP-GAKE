...
struct CryptoPrimitives {
    ...
    CryptoPrimitives() : 
        params(ASN1::secp160r1()), // 160-bit 椭圆曲线 (80-bit security)
    ...
    std::string Hash(const std::string& input) {
        SHA1 hash; // SHA1 (80-bit security)
        std::string digest;
...
public:
    ...
    Broadcast_f round1_generate_f() {
        ...
        k_i = SecByteBlock((const byte*)k_i_str.data(), 16); // 16-byte key for 2TDEA
        
        std::string y_prime_i_str = crypto.Hash(PointToString(my_file.Y_i) + id);
        SecByteBlock k_R_i((const byte*)y_prime_i_str.data(), 16);
    ...
    Broadcast_Alpha_X round2_generate_alpha_X(
        ...
        auto parse_message = [&](const std::string& decrypted_payload) -> DecryptedFMessage {
            ...
            if (parts.size() != 6) {
                throw std::runtime_error("Invalid message format: expected 6 parts");
            }
...
int main() {
    try {
        // 这个值应该在编译时由脚本定义，但我们为手动运行提供一个默认值。
