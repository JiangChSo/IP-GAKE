...
#include <tuple>
#include <sstream>
#include <chrono> // <-- 新增: 包含计时库

// Crypto++ Headers
#include <cryptopp/osrng.h>
#include <cryptopp/eccrypto.h>
#include <cryptopp/oids.h>
#include <cryptopp/sha.h>      // 使用 SHA224
#include <cryptopp/hex.h>
#include <cryptopp/integer.h>
#include <cryptopp/aes.h>      // 使用 AES
#include <cryptopp/modes.h>    // 使用 CBC 模式
#include <cryptopp/filters.h>  // 使用 StreamTransformationFilter
...
struct CryptoPrimitives {
    ...
    CryptoPrimitives() : 
        params(ASN1::secp224r1()), // 使用 224-bit 椭圆曲线
    ...
    std::string Encrypt(const SecByteBlock& key, const std::string& plaintext) {
        ...
        SecByteBlock iv(AES::BLOCKSIZE); // IV 尺寸为 16 字节
...
public:
    ...
    Broadcast_Alpha_X round2_generate_alpha_X(
        ...
        SecByteBlock k_prev((const byte*)k_prev_str.data(), 16);
        ...
        SecByteBlock k_next((const byte*)k_next_str.data(), 16);
        ...
        if (expected_c_prev_hex != msg_prev.c) throw std::runtime_error("c value check failed");
        ...
        if (expected_c_next_hex != msg_next.c) throw std::runtime_error("c value check failed");
        ...
        auto decrypt_and_store_R = [&](const DecryptedFMessage& msg, const PasswordFile& file) {
            ...
            SecByteBlock k_R((const byte*)y_prime_str.data(), 16);
            ...
            if (r_parts.size() != 2) throw std::runtime_error("Invalid decrypted R format");
    ...
    void round4_verify_auths_and_finalize(
        ...
        for(const auto& p : all_auths) {
            ...
            if (expected_auth != received_auth) {
                throw std::runtime_error("Auth verification failed");
            }
        }
...
int main() {
    try {
        // 【修改】这个值现在由编译脚本通过 -DNUM_PARTICIPANTS=... 标志定义
        #ifndef NUM_PARTICIPANTS
        #define NUM_PARTICIPANTS 4 // 为手动编译提供一个默认值
        #endif
        ...
        // 【修正】确保输出字符串与 Bash 脚本中的 grep 命令完全匹配
        std::cout << "\n--- Timing Results ---\n";
        std::cout << "Total Setup computation time: " << setup_duration.count() << " ms\n";
        std::cout << "Total Key Exchange computation time: " << exchange_duration.count() << " ms\n";
