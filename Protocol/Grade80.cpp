...
struct CryptoPrimitives {
    ...
    std::string Hash(const std::string& input) {
        SHA1 hash; // SHA-1 (80-bit security)
        std::string digest;
    ...
    std::string Encrypt(const SecByteBlock& key, const std::string& plaintext) {
        CBC_Mode<DES_EDE2>::Encryption e; // <-- 修改: 使用 CBC 模式
        SecByteBlock iv(DES_EDE2::BLOCKSIZE); // IV 尺寸为 8 字节
        AutoSeededRandomPool prng;
        prng.GenerateBlock(iv, iv.size());
        std::string ciphertext;
        e.SetKeyWithIV(key, key.size(), iv, iv.size());
        // <-- 修改: 使用 StreamTransformationFilter 进行非认证加密
    ...
    std::string Decrypt(const SecByteBlock& key, const std::string& iv_and_ciphertext) {
        ...
        CBC_Mode<DES_EDE2>::Decryption d; // <-- 修改: 使用 CBC 模式
        std::string decryptedtext;
        d.SetKeyWithIV(key, key.size(), iv, iv.size());
        // <-- 修改: 使用 StreamTransformationFilter 进行非认证解密
...
public:
    ...
    Broadcast_f round1_generate_f() {
        ...
        k_i = SecByteBlock((const byte*)k_i_str.data(), 16); // 密钥长度为 16 字节 (128-bit for 2TDEA)
        
        // 【修改】使用派生密钥 Y_i' = H(Y_i, id_i) 来加密 R_i
        std::string y_prime_i_str = crypto.Hash(PointToString(my_file.Y_i) + id);
        SecByteBlock k_R_i((const byte*)y_prime_i_str.data(), 16); // 密钥长度为 16 字节
        ...
        // 【修改】新的消息格式: ssid|id|X_hex_x|X_hex_y|enc_R_i_hex|c_hex
    ...
    Broadcast_Alpha_X round2_generate_alpha_X(
        ...
        SecByteBlock k_prev((const byte*)k_prev_str.data(), 16); // 密钥长度为 16 字节
        ...
        SecByteBlock k_next((const byte*)k_next_str.data(), 16); // 密钥长度为 16 字节
        ...
        // (7) 【修改】从解密后的消息中解析 ssid, id, X, enc_R_hex, c
        ...
        // (9) 【修改】使用 Y'_j = H(Y_j, id_j) 作为密钥来解密 R_j
        auto decrypt_and_store_R = [&](const DecryptedFMessage& msg, const PasswordFile& file) {
            ...
            SecByteBlock k_R((const byte*)y_prime_str.data(), 16); // 密钥长度为 16 字节
    ...
    // 【新增】轮次 4: 验证所有 Auth 消息, 并在成功后最终确定会话密钥
    void round4_verify_auths_and_finalize(
        ...
        // 协议的一个关键特性是，所有诚实的参与者最终都会计算出相同的 A 和 beta 值。
...
int main() {
    try {
        ...
        for (int i = 1; i <= NUM_PARTICIPANTS; ++i) {
            ...
            std::string pass = "password_" + std::to_string(i); // 使用唯一的密码
