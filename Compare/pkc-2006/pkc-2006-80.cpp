...
// Using SHA1 for 80-bit security level
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        players[i].auth_i = sha1_hash(round4_payload_base + ECPPointToString(params, players[i].K_i) + std::to_string(i));
    }
    ...
    for (int i = 0; i < n; ++i) {
        players[i].session_key = sha1_hash(round5_payload_base + ECPPointToString(params, players[i].K_i));
    }
