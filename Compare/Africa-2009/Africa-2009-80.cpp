...
// Generic SHA1 hashing function for 80-bit security
...
void encrypt_tweakable(const std::string& password, const std::string& tweak, const std::string& plaintext, std::string& ciphertext) {
    ...
    HKDF<SHA1> hkdf; // Using SHA1 for KDF to match security level
...
void decrypt_tweakable(const std::string& password, const std::string& tweak, const std::string& ciphertext, std::string& plaintext) {
    SecByteBlock derived(AES::DEFAULT_KEYLENGTH + AES::BLOCKSIZE);
    HKDF<SHA1> hkdf; // Using SHA1 for KDF
    hkdf.DeriveKey(derived, derived.size(), (const byte*)password.data(), password.size(), (const byte*)tweak.data(), tweak.size(), nullptr, 0);
...
struct Player {
    ...
    ECDSA<ECP, SHA1>::PrivateKey sig_sk; // Signature uses SHA1
    ECDSA<ECP, SHA1>::PublicKey sig_vk;
...
int main() {
    ...
    // Using secp160r1 for 80-bit security level
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // Initialize signature keys with the 80-bit curve
        players[i].sig_sk.Initialize(prng, ASN1::secp160r1());
        ...
        players[i].c_i = sha1_hash(ssid + players[i].z_i_star + vk_str + std::to_string(i));
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        players[i].h_i = sha1_hash(ECPPointToString(params, players[i].Z_i));
        players[i].h_i_plus_1 = sha1_hash(ECPPointToString(params, players[i].Z_i_plus_1));
        ...
        players[i].c_i_prime = sha1_hash(ssid_prime + players[i].X_i + std::to_string(i));
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        players[i].authenticator = sha1_hash(auth_payload);

        ECDSA<ECP, SHA1>::Signer signer(players[i].sig_sk);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < n; ++j) {
            ECDSA<ECP, SHA1>::Verifier verifier(players[j].sig_vk);
        ...
        if (all_sigs_valid) {
            ...
            players[i].session_key = sha1_hash(key_payload);
        } else {
