...
// --- Type Aliases for Clarity (Corrected) ---
...
std::string IntegerToString(const Integer& i) {
    std::ostringstream oss;
    oss << i;
    return oss.str();
}

// Simulates the tweakable cipher using HKDF to derive a key and IV from password and tweak
...
struct Player {
    ...
    ECDSA<ECP, SHA256>::PrivateKey sig_sk;
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        players[i].sig_sk.Initialize(prng, ASN1::secp256r1());
